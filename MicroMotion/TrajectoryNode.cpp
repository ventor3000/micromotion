...
bool TrajectoryNode_BeginDwell(struct TTrajectoryNode* n);
bool TrajectoryNode_UpdateDwell();
...
static long dwellMillisecondsLeft=0;
...
bool TrajectoryNode_Begin(struct TTrajectoryNode* n, struct TMachine* m) {
	...
	if(n==NULL) { // if we executes NULL nodes we are done right away
    //HAL_SetTimer(1000); // free running timer at 1000 times a second, waiting for new commands
    return true;
	}
  ...
  case TRN_DWELL:
    return TrajectoryNode_BeginDwell(n);
	default:
...
bool TrajectoryNode_Update(struct TTrajectoryNode* n, struct TMachine* m) {
  ...
  case TRN_DWELL:
    return TrajectoryNode_UpdateDwell();
	case TRN_SET_DIGITAL:
...
bool TrajectoryNode_BeginDwell(struct TTrajectoryNode* n) {
  dwellMillisecondsLeft=n->milliseconds;
  HAL_SetTimer(1000); // tick every 1000 microseconds ie. once every milliscecond
  return dwellMillisecondsLeft<=00;
}

bool TrajectoryNode_UpdateDwell() {
  dwellMillisecondsLeft--;
  return dwellMillisecondsLeft<=0;
}
