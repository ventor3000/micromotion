...
COORDINATE DDA_Init(struct TMotor* _motors, COORDINATE* end) {
	...
	for (int i = 0; i < AXIS_COUNT; i++) {
		...
		absDelta = absValue(end[i] - motors[i].position);

		//point[i] = start[i];
		errorDelta[i] = absDelta << 1;
		Motor_SetDirection(&motors[i],(end[i]>motors[i].position) ? 1 : -1);
